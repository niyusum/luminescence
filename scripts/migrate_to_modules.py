...
USAGE:
  ...
  python migrate_to_modules.py apply   # Execute migration
  python migrate_to_modules.py revert  # Undo migration
...
def plan_migration() -> Tuple[Dict[Path, Path], List[str]]:
    ...
    for filename, old_path in files.items():
        if "core" in old_path.parts or "modules" in old_path.parts:
            continue
...
def create_init_files(modules_dir: Path):
...
def cmd_plan():
    ...
    print("Run 'python migrate_to_modules.py apply' to execute")
...
def cmd_apply():
    ...
    print("\n✓ MIGRATION COMPLETE\n")
    print("To revert: python migrate_to_modules.py revert")
