...
def ratelimit(
    ...
    Example:
        ...
        >>> @ratelimit(uses=3, per_seconds=3600, command_name="announce", scope="guild")
        >>> async def announce(self, inter: discord.Interaction):
    ...
    def decorator(func: F) -> F:
        ...
        async def wrapper(self, inter: discord.Interaction, *args, **kwargs):
            ...
            if scope == "guild":
                ...
                scope_id = inter.guild.id
                scope_name = f"guild:{inter.guild.name}"
            else:
                scope_id = inter.user.id
                scope_name = f"user:{inter.user.name}"
            ...
            if inter.guild:
                context_kwargs["guild_id"] = inter.guild.id
            ...
            async with LogContext(**context_kwargs):
                try:
                    ...
                    if current and int(current) >= uses:
                        ...
                        logger.warning(
                            f"Rate limit exceeded for {command_name} "
                    ...
                    if current:
                        ...
                        logger.debug(
                            f"Rate limit check passed for {command_name} "
                        ...
                        logger.debug(
                            f"Rate limit initialized for {command_name} "
                    ...
                    logger.error(
                        f"Rate limit check failed for {command_name} "
