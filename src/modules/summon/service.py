...
class SummonService:
    ...
    def roll_maiden_tier(player_level: int) -> int:
        ...
        # Use cryptographically secure RNG to prevent summon result prediction
        selected_tier = secrets.SystemRandom().choices(tiers, weights=weights, k=1)[0]
    ...
    async def perform_summon(
        ...
        if is_pity:
            ...
            maiden_base = secrets.choice(maiden_bases)
    ...
    async def check_and_trigger_pity(
        ...
        if unowned:
            # Use cryptographically secure random selection for pity maiden
            maiden_base = secrets.choice(unowned)
            ...
            if not candidates:
                ...
                tier = next_tier
            # Use cryptographically secure random selection
            maiden_base = secrets.choice(candidates)
