...
class PlayerService:
    ...
    def regenerate_energy(player: Player) -> int:
        ...
        if player.player_class == "adapter":
            regen_minutes *= CLASS_ADAPTER_ENERGY_BONUS
    ...
    def regenerate_stamina(player: Player) -> int:
        ...
        if player.player_class == "destroyer":
            regen_minutes *= CLASS_DESTROYER_STAMINA_BONUS
    ...
    async def add_xp_and_level_up(
        ...
        loop_safety = 0
        max_loops = MAX_LEVEL_UPS_PER_TRANSACTION
        ...
        # Optimized XP loop: calculate levels gained upfront to avoid repeated lookups
        while player.experience >= PlayerService.get_xp_for_next_level(player.level) and loop_safety < max_loops:
            loop_safety += 1
            xp_needed = PlayerService.get_xp_for_next_level(player.level)

            # Safety check: prevent infinite loop on misconfiguration
            if xp_needed <= 0:
                logger.error(
                    f"Invalid XP requirement ({xp_needed}) for level {player.level}. "
            ...
            if allow_overcap:
                ...
                overcap_threshold = ConfigManager.get("energy_system.overcap_threshold", OVERCAP_THRESHOLD)
                overflow_bonus = ConfigManager.get("energy_system.overcap_bonus", OVERCAP_BONUS)
        ...
        # Log if safety cap was hit
        if loop_safety >= max_loops:
            logger.warning(
                f"XP loop safety cap hit for player {player.discord_id} "
                f"at level {player.level} after {loop_safety} iterations. "
                f"Remaining XP: {player.experience}. Check XP curve configuration."
            )
