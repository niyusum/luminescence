    ...
    async def attack_matron(
        ...
        if victory:
            ...
            stmt = select(SectorProgress).where(
                SectorProgress.player_id == player.discord_id,
                SectorProgress.sector_id == matron["sector_id"],
                SectorProgress.sublevel == matron["sublevel"]
            )
            result_progress = await session.execute(stmt)
            sector_progress = result_progress.scalar_one_or_none()
            if sector_progress:
