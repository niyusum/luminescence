...
class AscensionService:
    ...
    def _determine_token_drop(floor: int) -> Dict[str, Any]:
        ...
        if floor <= 10:
            ...
            # 60% bronze, 40% silver
            return {"type": random.choices(["bronze", "silver"], weights=[60, 40])[0], "quantity": 1}
        elif floor <= 50:
            # 70% silver, 30% gold
            return {"type": random.choices(["silver", "gold"], weights=[70, 30])[0], "quantity": 1}
        elif floor <= 100:
            # 60% gold, 40% platinum
            return {"type": random.choices(["gold", "platinum"], weights=[60, 40])[0], "quantity": 1}
        elif floor <= 150:
            # 70% platinum, 30% diamond
            return {"type": random.choices(["platinum", "diamond"], weights=[70, 30])[0], "quantity": 1}
        else:
            # 80% diamond, 20% mythic (future token type)
            return {"type": random.choices(["diamond"], weights=[100])[0], "quantity": 1}
